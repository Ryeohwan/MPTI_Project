...
const MySchedule =()=> {
    ...
    const today = parseInt(new Date().toLocaleString().split('. ').slice(0,3).reduce((prev, next)=>next.length<2?prev+'0'+next:prev+next,''))
    useEffect(()=>{
        async function getSchedule(){
            ...
            data.length && data.map((item) => {
                const lessonDay = parseInt(item.year.toString() + (item.month<10?'0'+item.month:item.month) + (item.day<10?('0'+item.day):item.day));
                console.log(lessonDay, today);
                lessonDay>=today?(lessonDay===today?temp_data1.push(item):temp_data2.push(item)) :temp_data3.push(item);
                return null;
            })
            console.log('오늘:', temp_data1, '이후수업:', temp_data2, '끝난 수업:', temp_data3)
    ...
    return (
        <div className={styles.schedule}>
            ...
            <ul className={styles.card_list_bottom}>
                {otherSchedule.length ? otherSchedule.map((item, index) => <ScheduleCardItem key={index} schedule={item} active={false}/>) : <div>수업을 예약해 주세요!!</div>}
            </ul>
        </div>
    );
};
