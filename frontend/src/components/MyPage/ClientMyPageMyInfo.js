...
// userInfo({name:'이름', gender:'성별', })
    ...
    async function infoEdit(e){
        ...
        axios.post(update_url, {email:userInfo.email, password:password, phone:phone}).then((data)=> {
    ...
    function editCheck(){
        if(document.getElementById('phone').value === userInfo.phone){
            setEdit(false)
        }
        else{
            setShowModal(true)
        ...
        <div className={styles.container}> 
            ...
            <form className={styles.out_box} method='PUT' onSubmit={(e) => {e.preventDefault()}}>
                <div className={styles.in_box}>
                    ...
                    {Object.values(userInfo).map((value, index)=>
